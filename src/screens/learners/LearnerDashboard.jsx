...
import { 
  ...
  Wallet, History, ArrowRight, Eye, EyeOff
} from 'lucide-react';
    ...
    <div style={{ 
      ...
      <div className="mb-4 flex justify-center">
        <div className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full border ${
          ...
          <span className="text-sm font-semibold" style={!isDarkMode ? { color: headingColor } : {}}>
            {isSecondaryStudent ? '🎓 Secondary School' : '📚 Primary School'}
          </span>
          <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-white/50 dark:bg-white/10" style={!isDarkMode ? { color: darkTeal } : {}}>
        ...
        <div className={`mb-6 rounded-2xl p-4 border ${
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className={`font-bold ${isDarkMode ? 'text-teal-300' : 'text-teal-800'}`} style={!isDarkMode ? { color: headingColor } : {}}>
                  Daily Bonus Waiting!
                </h3>
            ...
            <button
              ...
              className={`px-5 py-2 rounded-xl text-xs font-black tracking-wider uppercase transition-all shadow-sm ${
                isDarkMode 
      ...
      <div className="text-center mb-6">
        <h2 
          className={`font-bold ${isDarkMode ? 'text-white' : 'text-[#19475B]'}`}
          style={{ fontSize: !isDarkMode ? `${headingSize}px` : '15px', color: !isDarkMode ? headingColor : undefined }}
        >
          Select a Game
        </h2>
        <p 
          className={`mt-1 ${isDarkMode ? 'text-slate-400' : 'text-[#19475B]/70'}`}
          style={{ fontSize: '12px', color: !isDarkMode ? accentColor : undefined }}
        >
              ...
              >
                <div className="flex-1 flex items-center justify-center min-h-0 py-0">
                  <img 
                    src={game.image}
                    alt={game.title}
                    className={`w-auto h-full max-h-[120px] object-contain transition-all duration-300 ${
                      isHovered ? 'scale-110' : 'hover:scale-105'
                    } drop-shadow-lg`}
                  />
                </div>
                <div className={`flex justify-between items-center mt-0 flex-shrink-0 transition-all duration-300 ${
                  isHovered ? 'opacity-100' : ''
                }`}>
                  <div className={`rounded-full px-2 py-0.5 inline-block transition-all duration-300 ${
                    isHovered ? 'bg-white/30' : 'bg-white/20'
                  }`}>
                    <span className={`text-white text-[10px] font-semibold transition-all duration-300 ${
                      isHovered ? 'text-white' : ''
                    }`}>
                      {isSecondaryStudent ? 'Browse Secondary' : 'Play Now'}
                    </span>
                  </div>
                  <div className={`text-white font-black text-sm tracking-wider transition-all duration-300 ${
                    isHovered ? 'text-white' : ''
                  }`}>
                    {game.title}
                  </div>
  ...
  const renderRewardsContent = () => (
    <div style={{ fontFamily: `${fontFamily}, "Segoe UI", Calibri, "Trebuchet MS", sans-serif`, color: bodyColor }}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
        <div className="rounded-lg p-3 text-center border" style={!isDarkMode ? { backgroundColor: '#0f5f5a', borderColor: '#0a4a45', borderRadius: `${borderRadius}px` } : { backgroundColor: 'rgb(30 41 59)', borderColor: 'rgb(51 65 85)' }}>
          <p className="text-[10px] font-medium uppercase tracking-wider" style={!isDarkMode ? { color: '#bcefe6' } : { color: '#94a3b8' }}>Points</p>
          <p className="text-lg font-bold" style={!isDarkMode ? { color: '#f7fffd' } : { color: '#f1f5f9' }}>{formatPoints(points.current)}</p>
        </div>
        <div className="rounded-lg p-3 text-center border" style={!isDarkMode ? { backgroundColor: '#0d5652', borderColor: '#0a4a45', borderRadius: `${borderRadius}px` } : { backgroundColor: 'rgb(30 41 59)', borderColor: 'rgb(51 65 85)' }}>
          <p className="text-[10px] font-medium uppercase tracking-wider" style={!isDarkMode ? { color: '#bcefe6' } : { color: '#94a3b8' }}>Available</p>
          <p className="text-lg font-bold" style={!isDarkMode ? { color: '#f7fffd' } : { color: '#f1f5f9' }}>{rewards.filter(r => r.stock_quantity === undefined || r.stock_quantity > 0).length}</p>
        </div>
        <div className="rounded-lg p-3 text-center border" style={!isDarkMode ? { backgroundColor: '#0b4f4c', borderColor: '#093f3b', borderRadius: `${borderRadius}px` } : { backgroundColor: 'rgb(30 41 59)', borderColor: 'rgb(51 65 85)' }}>
          <p className="text-[10px] font-medium uppercase tracking-wider" style={!isDarkMode ? { color: '#bcefe6' } : { color: '#94a3b8' }}>Total Rewards</p>
          <p className="text-lg font-bold" style={!isDarkMode ? { color: '#f7fffd' } : { color: '#f1f5f9' }}>{rewards.length}</p>
        </div>
        <div className="rounded-lg p-3 text-center border" style={!isDarkMode ? { backgroundColor: '#094845', borderColor: '#073a37', borderRadius: `${borderRadius}px` } : { backgroundColor: 'rgb(30 41 59)', borderColor: 'rgb(51 65 85)' }}>
          <p className="text-[10px] font-medium uppercase tracking-wider" style={!isDarkMode ? { color: '#bcefe6' } : { color: '#94a3b8' }}>Vouchers</p>
          <p className="text-lg font-bold" style={!isDarkMode ? { color: '#f7fffd' } : { color: '#f1f5f9' }}>{redemptions.length}</p>
        </div>
      </div>
      ...
      <div className="flex gap-2 mb-4">
        <button
          ...
          className={`px-4 py-2 rounded-lg text-sm font-bold transition-all duration-200 flex items-center gap-2 border-2 ${
      ...
      {activeStoreTab === 'vouchers' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between flex-wrap gap-2 p-3 rounded-lg border" style={!isDarkMode ? { backgroundColor: `${cardBg}60`, borderColor: `${accentColor}30`, borderRadius: `${borderRadius}px` } : { backgroundColor: 'rgb(30 41 59)', borderColor: 'rgb(51 65 85)' }}>
            <div>
              <h2 className="font-bold flex items-center gap-2" style={!isDarkMode ? { color: headingColor } : { color: '#f1f5f9' }}>
                <Award className="w-5 h-5" style={!isDarkMode ? { color: accentColor } : {}} />
                My Vouchers
              </h2>
            ...
            <form onSubmit={handleSearch} className="flex gap-2 max-w-md">
              <div className="relative flex-1">
                <input
                  ...
                  className="w-full px-3 py-2 pl-9 pr-8 rounded-lg text-sm focus:outline-none focus:ring-2 transition border-2"
                  style={!isDarkMode ? { 
                ...
                />
                <Search className="absolute left-2.5 top-1/2 transform -translate-y-1/2 w-4 h-4" style={!isDarkMode ? { color: bodyColor } : { color: '#64748b' }} />
                {searchInput && (
                  <button
                    type="button"
              ...
              <button
                type="submit"
                className="px-5 py-2 rounded-lg text-sm font-medium text-white transition border-2"
                style={{ backgroundColor: accentColor, borderColor: accentColor }}
              >
                Search
              </button>
...
>
  {/* Header Section */}
  <div
    className="p-4 border-b-2 flex items-center justify-between border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900"
  ...
  >
    ...
    <div className="flex items-center gap-2">
      ...
      {/* Item Counter Badge */}
      <span
        className="px-3 py-1 rounded-full text-[10px] font-semibold border-2"
        style={{
        ...
        {[...Array(8)].map((_, i) => (
          ...
          >
            <div className="w-full h-full flex items-center justify-center">
              <div
                className="w-12 h-12 rounded-full bg-slate-300 dark:bg-slate-600"
          ...
          return (
            ...
            >
              <div className="p-3.5 flex flex-col h-full">
                {/* Image Container */}
                <div
                  className="w-full aspect-square rounded-xl overflow-hidden flex items-center justify-center mb-3 border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900"
                ...
                {/* Reward Title */}
                <p
                  className="text-sm font-bold text-center leading-tight min-h-[36px]"
                ...
                {/* Points Required */}
                <div
                  className="mt-2 inline-flex items-center justify-center gap-1 self-center rounded-full px-3 py-1 text-xs font-semibold"
                  style={{
                ...
                {reward.stock_quantity !== undefined && (
                  <div className="text-[11px] font-medium mt-2 text-center" style={{ color: stockStatus.color }}>
                    {stockStatus.label}
                  </div>
                )}
              </div>
  ...
  const renderBadgesContent = () => (
    <div className={`rounded-2xl p-8 border ${
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className={`text-2xl font-bold ${isDarkMode ? 'text-white' : 'text-[#19475B]'}`} style={!isDarkMode ? { color: headingColor } : {}}>My Badges</h2>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
          {badges.map((badge) => (
            ...
            >
              ...
              <h4 className={`font-bold text-sm ${isDarkMode ? 'text-white' : 'text-[#19475B]'}`} style={!isDarkMode ? { color: headingColor } : {}}>
                {badge.name}
              </h4>
              {badge.description && (
                <p className={`text-[10px] mt-1 ${isDarkMode ? 'text-slate-400' : 'text-[#19475B]/70'}`} style={!isDarkMode ? { color: bodyColor } : {}}>
                  {badge.description}
                </p>
              )}
            </div>
  ...
  const renderHistoryContent = () => (
    <div className={`rounded-2xl p-8 border text-center ${
      ...
      <div className="text-6xl mb-4">📊</div>
      <h2 className={`text-2xl font-bold mb-2 ${isDarkMode ? 'text-white' : 'text-[#19475B]'}`} style={!isDarkMode ? { color: headingColor } : {}}>Quiz History</h2>
      <p className={`mb-6 ${isDarkMode ? 'text-slate-400' : 'text-[#19475B]/70'}`} style={!isDarkMode ? { color: bodyColor } : {}}>
        View your past quiz performances and track your progress
      </p>
      <button
        onClick={() => navigate('/quiz-history')}
        className="px-6 py-3 bg-gradient-to-r from-teal-600 to-cyan-600 text-white rounded-xl font-bold hover:shadow-lg transition-all"
        style={!isDarkMode ? { backgroundColor: accentColor, backgroundImage: 'none' } : {}}
      >
        View History
      </button>
  ...
  const renderLeaderboardContent = () => (
    <div className={`rounded-2xl border overflow-hidden ${
      ...
      <div className={`p-5 border-b ${isDarkMode ? 'border-teal-400/20' : 'border-teal-200/50'}`} style={!isDarkMode ? { borderColor: `${accentColor}40` } : {}}>
        <div className="flex items-center justify-between">
          <div>
            <h2 className={`text-xl font-bold ${isDarkMode ? 'text-white' : 'text-[#19475B]'}`} style={!isDarkMode ? { color: headingColor } : {}}>🏆 Leaderboard</h2>
          ...
          {leaderboard.slice(0, 20).map((learner, idx) => {
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className={`text-sm font-semibold truncate ${isMe ? 'text-teal-600' : isDarkMode ? 'text-white' : 'text-gray-800'}`} style={!isDarkMode ? { color: isMe ? accentColor : bodyColor } : {}}>
                    {learner.full_name || learner.username}
                    {isMe && <span className="ml-1.5 text-xs text-teal-500" style={!isDarkMode ? { color: accentColor } : {}}>(You)</span>}
                  </p>
                  <p className={`text-xs ${isDarkMode ? 'text-slate-500' : 'text-gray-400'}`} style={!isDarkMode ? { color: bodyColor } : {}}>
                ...
                <div className="text-right flex-shrink-0">
                  <p className={`text-sm font-black ${isDarkMode ? 'text-teal-400' : 'text-teal-600'}`} style={!isDarkMode ? { color: accentColor } : {}}>
                    {(learner.lifetime_points || 0).toLocaleString()}
                  </p>
                  <p className={`text-[10px] ${isDarkMode ? 'text-slate-500' : 'text-gray-400'}`} style={!isDarkMode ? { color: bodyColor } : {}}>points</p>
                </div>
              </div>
            );
          })}
    ...
    >
        ...
        >
          <div className="px-4 py-3">
            <div className="flex items-center justify-between mb-2">
              <div>
                <h2 className="font-bold text-lg" style={{ color: iceWhite }}>
                  {getGreeting()}, {user?.fullName?.split(' ')[0] || 'Learner'}! 👋
                </h2>
                <p className="text-xs" style={{ color: iceWhite }}>
                  {formatDate(currentDateTime)} • {formatTime(currentDateTime)}
                </p>
              </div>
              <div className="flex items-center gap-3">
                <div className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/10 backdrop-blur-sm border border-white/20">
                  <span className="text-xs" style={{ color: iceWhite }}>⭐</span>
                  <span className="font-bold text-sm" style={{ color: iceWhite }}>
                    {points.current} pts
                  </span>
                </div>
                <button
            ...
            <div className="grid grid-cols-4 gap-2">
              <div className="bg-white/10 backdrop-blur-sm rounded-xl px-3 py-2 text-center border border-white/10">
                <p className="text-[10px] font-medium uppercase tracking-wider" style={{ color: iceWhite }}>Lifetime</p>
                <p className="font-bold text-base" style={{ color: iceWhite }}>{points.lifetime}</p>
              </div>
              <div className="bg-white/10 backdrop-blur-sm rounded-xl px-3 py-2 text-center border border-white/10">
                <p className="text-[10px] font-medium uppercase tracking-wider" style={{ color: iceWhite }}>Streak</p>
                <p className="font-bold text-base" style={{ color: iceWhite }}>{streak} 🔥</p>
              </div>
              <div className="bg-white/10 backdrop-blur-sm rounded-xl px-3 py-2 text-center border border-white/10">
                <p className="text-[10px] font-medium uppercase tracking-wider" style={{ color: iceWhite }}>Rank</p>
                <p className="font-bold text-base" style={{ color: iceWhite }}>{getRankTitle()}</p>
              </div>
              <div className="bg-white/10 backdrop-blur-sm rounded-xl px-3 py-2 text-center border border-white/10">
                <p className="text-[10px] font-medium uppercase tracking-wider" style={{ color: iceWhite }}>Badges</p>
                <p className="font-bold text-base" style={{ color: iceWhite }}>{badges.length}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {showLevelModal && learnerProgress && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => setShowLevelModal(false)}>
          <div className={`rounded-2xl max-w-lg w-full max-h-[85vh] flex flex-col shadow-xl border ${
            isDarkMode 
            ...
            <div className="p-5 overflow-y-auto flex-1">
              <div className={`mb-6 text-center p-5 rounded-xl border ${
                isDarkMode 
                ...
                <p className={`text-2xl font-black ${isDarkMode ? 'text-teal-300' : 'text-[#19475B]'}`} style={!isDarkMode ? { color: headingColor } : {}}>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className={`text-sm font-bold ${
                            ...
                            {level.name}
                            {isSecondary && <span className="text-[10px] ml-1 text-purple-500">(Secondary)</span>}
                          </p>
                          {isCurrent && (
                            <p className={`text-[9px] font-semibold ${isDarkMode ? 'text-teal-300' : 'text-[#19475B]'}`} style={!isDarkMode ? { color: accentColor } : {}}>
                              In Progress
                            </p>
                          )}
                          {isCompleted && (
                            <p className={`text-[9px] font-medium ${isDarkMode ? 'text-emerald-300' : 'text-emerald-600'}`}>
                              Completed
                            </p>
                          )}
                        </div>
      ...
      {showAvatarModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => setShowAvatarModal(false)}>
          <div className={`rounded-2xl max-w-md w-full p-6 shadow-xl border ${
            isDarkMode 
              ? 'bg-slate-800 border-teal-400/30' 
              : 'bg-white border-teal-300/40'
          }`} style={!isDarkMode ? { backgroundColor: cardBg, borderColor: `${accentColor}40`, borderRadius: `${borderRadius}px` } : {}} onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className={`text-lg font-bold ${isDarkMode ? 'text-white' : 'text-[#19475B]'}`} style={!isDarkMode ? { color: headingColor } : {}}>
                Choose Your Avatar
              </h3>
              <button 
